...
export default function SearchBar({ data }: FuzzySearchProps) {
  ...
  return (
    <div className="relaive w-75 md:w-md">
      {/* Search Input Box */}
      ...
      {query !== "" && (
        <ul className="absolute w-75 md:w-md bg-white rounded-sm border shadow-sm max-h-80 overflow-y-auto divide-y divide-gray-100 z-50">
          {results.length > 0 ? (
            results.map((item) => (
              <Link
                href={item.link}
              ...
              >
                ...
                <p className="font-medium text-gray-100 text-xs">{item.author}</p>
              </Link>
            ))
          ) : (
