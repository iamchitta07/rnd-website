...
export default function SearchBar({ data }: FuzzySearchProps) {
  ...
  const fuse = useMemo(() => {
    return new Fuse(data, {
      keys: ["blog_title"], // Fields to look through
        ...
        <ul className="absolute w-75 md:w-xl xl:w-2xl bg-white rounded-sm border shadow-sm max-h-80 overflow-y-auto divide-y divide-gray-100 z-50 text-lg md:text-xl">
          {results.length > 0 ? (
            results.map((item) => (
              ...
              >
                <p className="font-medium text-gray-100">{item.blog_title}</p>
                <p className="font-medium text-gray-100 text-xs italic">{item.blog_description}</p>
              </Link>
            ))
          ) : (
